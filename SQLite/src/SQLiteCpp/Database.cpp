...
// Shortcut to test if a key of a table exists.
bool Database::keyExists(const char* apTableName, const char* apKeyName, const char* apKeyValue)
{
	std::ostringstream osQuery;
	osQuery << "SELECT count(*) FROM " << apTableName << " WHERE " << apKeyName << "= '" << apKeyValue << "'";

	Statement query(*this, osQuery.str());
	(void)query.executeStep(); // Cannot return false, as the above query always return a result
	int Nb = query.getColumn(0);
	return (1 == Nb);
}
