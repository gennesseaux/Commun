                                             ...
                                             CComVariant&    variant )
{
    ...
    else
    {
        ...
        case VT_UI2:
            variant.iVal = *(unsigned short*) entry;
            break;
        ...
        case VT_UINT:
            variant.intVal = *(unsigned int*) entry;
            break;
        
        case VT_INT:
            variant.intVal = *(int*) entry;
            break;

		case VT_UI4:
            variant.lVal = *(unsigned long*) entry;
            break;
...
inline void WriteDataToStream( const VARIANT* variant, IStream* pStream )
{
    ...
    else
    {
		...
		case VT_UINT:
			stream.Write(V_UINT(variant));
			break;
        
		case VT_INT:
			stream.Write(V_INT(variant));
			break;
        
		case VT_UI4:
            stream.Write( V_UI4( variant ) );
            break;
...
inline void ReadDataFromStream( VARTYPE vt, IStream* pStream, VARIANT& variant )
{
    ...
    else
    {
        ...
        case VT_I2:
 		...
 		case VT_UINT:
			stream.Read(V_UINT(&variant));
			break;
        
       case VT_INT:
            stream.Read( V_INT( &variant ) );
            break;
        
        case VT_UI4:
            stream.Read( V_UI4( &variant ) );
            break;
